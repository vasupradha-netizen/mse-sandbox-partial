...
export default class ImageCapture extends LightningElement {
  ...
  connectedCallback() {
    debug(`Working on ${this.objectApiName} with Id '${this.infoFetchedFromWoli.Id}'`);
  }
  ...
  async uploadData(fileName, description, fileData, woliId) {
    ...
    await this.createCdl(woliId, contentDocumentId);
  }

  async createCdl(woliId, contentDocumentId) {
    debug("Creating a CDL...");

    await createRecord({
      apiName: "ContentDocumentLink",
      fields: {
        LinkedEntityId: woliId,
